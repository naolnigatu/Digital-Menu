...
export default function SuperAdminView() {
  ...
  const handleSaveGlobalSettings = async (e: React.FormEvent) => {
    ...
    updateGlobalSettings({
      ...
      allowedDiningServiceTypes: allowedServiceTypes,
      allowedSubscriptionDurations: allowedDurationsText.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n)),
        ...
        <div className="space-y-4 animate-in fade-in duration-150">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <form onSubmit={handleSaveGlobalSettings} className="space-y-4 max-w-lg">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Platform-wide Allowed Subscription Durations (comma separated days)</label>
                <input
                  type="text"
                  value={allowedDurationsText}
                  onChange={(e) => setAllowedDurationsText(e.target.value)}
                ...
                <p className="text-[9px] text-slate-400 mt-1 leading-normal">
                  Define the durations in days that platform merchants are allowed to offer to their customers.
                </p>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Globally Allowed Dining Service Types</label>
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl grid grid-cols-2 gap-2">
                  {[
                    ...
                    const isChecked = allowedServiceTypes.includes(srv.id);
                    return (
                      <label key={srv.id} className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={() => {
                            if (isChecked) {
                              setAllowedServiceTypes(prev => prev.filter(x => x !== srv.id));
                            } else {
                              setAllowedServiceTypes(prev => [...prev, srv.id]);
                            }
                          }}
                ...
                <p className="text-[9px] text-slate-400 mt-1 leading-normal">
                  Enable or disable service types platform-wide. Disabled service types cannot be used by any business on the platform.
                </p>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Globally Allowed Payment Methods</label>
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl grid grid-cols-2 gap-2">
                  {[
                    { id: 'cash', label: 'Cash Payment' },
                    { id: 'stripe', label: 'Stripe Pay Online (Pro)' },
                    { id: 'mobile_money', label: 'Mobile Money' },
                    { id: 'bank_transfer', label: 'Bank Transfer' },
                    { id: 'binance_id', label: 'Binance Pay (ID)' },
                    { id: 'binance_wallet', label: 'Binance BEP20 Wallet (Pro)' }
                  ].map((pay) => {
                    const isChecked = allowedPaymentMethods.includes(pay.id);
                    return (
                      <label key={pay.id} className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={() => {
                            if (isChecked) {
                              setAllowedPaymentMethods(prev => prev.filter(x => x !== pay.id));
                            } else {
                              setAllowedPaymentMethods(prev => [...prev, pay.id]);
                            }
                          }}
                          className="rounded text-indigo-600 focus:ring-indigo-500"
                        />
                ...
                <p className="text-[9px] text-slate-400 mt-1 leading-normal">
                  Toggle payment methods permitted platform-wide. Merchants can only enable payment options approved here. Card option is permanently deprecated.
                </p>
              </div>
